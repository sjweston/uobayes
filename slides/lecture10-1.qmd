...
m3 <- brm(
  ...
  bf(chl | mi() ~ 1) + 
  set_rescor(FALSE),
  prior = c( prior(normal(0,10), class=Intercept, resp=bmi),
             prior(normal(0,10), class=Intercept, resp=chl),
             prior(normal(0,10), class=b),
             prior(exponential(1), class=sigma, resp=bmi),
             prior(exponential(1), class=sigma, resp=chl) ),
  iter=2000, warmup=500, chains=4, cores=4, seed=1,
  save_pars = save_pars(latent=T),
  file = here("files/models/m101.3")           
)
```
