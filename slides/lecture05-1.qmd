...
m1 = brm(
  ...
  seed = 3,
  file = here("files/data/generated_data/m5.1")
)
```
...
m2 = brm(
  ...
  iter = 5000, warmup = 1000, chains = 4, 
  seed = 3,
  file = here("files/data/generated_data/m5.2")
)
...
```{r}
#| code-fold: true
summary(m1)$fixed %>% round(2) %>% select(Estimate, contains("95"))
```
:::
...
```{r}
#| code-fold: true
summary(m2)$fixed %>% round(2) %>% select(Estimate, contains("95"))
```
:::
:::::
...
m3 <- brm(
  ...
  iter = 5000, warmup = 1000, chains = 4, 
  seed = 3,
  file = here("files/data/generated_data/m5.3")
)
```
...
m4 <- brm(
  ...
  iter = 5000, warmup = 1000, chains = 4, 
  seed = 3,
  file = here("files/data/generated_data/m5.4")
)
```
...
Recreate this figure (from the model without department)
