...
\begin{align*}
R_{ij} &\sim \text{Categorical}(p) \\
\text{logit}(p_k) &= \alpha_{k,j} - \phi \\
\phi &= 0 \\
\alpha_{k,j} &= \bar{\alpha}_k + U_{kj} \\
\bar{\alpha} &\sim \text{Normal}(0, 1.5) \\
\sigma_{\alpha_k} &\sim{Exponential}(1)
\end{align*}
...
m1 <- brm(
  ...
  prior = c( prior(normal(0, 1.5), class = Intercept),
             prior(expoential(1), class=sd) ),
  iter=2000, warmup=1000, seed=3, cores=4,
  file = here("files/models/m91.1b")
)
```

---

### model 2: age and wave
...
\alpha_{k,j} &= \bar{\alpha}_k + U_{kj} \\
\bar{\alpha}_k &\sim \text{Normal}(0, 1.5) \\
...
```{r}
m3 %>% spread_draws( sdgp_a_gpage, lscale_a_gpage )
```
...
loo_compare(`m1: intercept only` = m1, m2, `m3: gaussian proces`  = m3)
```
...
pred_m2 = nd %>% 
  add_epred_draws(m2, allow_new_levels = T) %>% 
  mutate(model = "m2") 

pred_m3 = nd %>% 
  add_epred_draws(m3, allow_new_levels = T) %>% 
  mutate(model = "m3")
...
post %>% 
  ...
  geom_line(aes(group = .draw),
            linewidth = 1/4, alpha = 1/4, color = "#1c5253") +
  stat_function(fun = function(x) mean(post$sdgp_gpday)^2 *
                  exp(-(1 / (2 * mean(post$lscale_gpday)^2)) * x^2),
                color = "#0f393a", linewidth = 1) +
