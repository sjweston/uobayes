...
## Practice Time: Analyzing Sleep Data
...
# Now plot some prior predictive regression lines
```
...
### Understanding Entropy Visually

Let's visualize what we mean by high and low entropy distributions:
...
ggplot(entropy_data, aes(x = value)) +
  geom_density(fill = "#1c5253", alpha = 0.5) +
  facet_wrap(~type) +
...
The low entropy distribution shows more certainty - most values are concentrated around a single point. The high entropy distribution shows more uncertainty - values are more evenly spread across the range. This concept is important when we think about our posterior distributions: tighter distributions (low entropy) indicate more certainty in our parameter estimates.
