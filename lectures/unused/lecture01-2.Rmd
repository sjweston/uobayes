...
```{r, echo = F, fig.retina = 3, fig.height = 4}
d <-
  tibble(p1 = 0,
...
Suppose we draw 3 marbles, with replacement, from this bag. For any one of these possibilities, we can identify every possible draw.

---

What is every possible outcome when there is 1 blue marble and 3 white marbles?
...
Going back to our garden, we can count the number of pathways that result in this specific pattern of draws.
...
Note that we can generalize this into a formula like:

$$
N_b\times N_w \times N_b = N_b^2N_w
$$
...
Return to the globe example. Assume a prior for $p$ that is equal to zero when $p < 0.5$ and is a positive constant when p $≥ 0.5$. Again compute and plot the grid approximate posterior distribution for each of the following sets of observations:
